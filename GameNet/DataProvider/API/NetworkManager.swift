...
public class NetworkManager {
    ...
    @discardableResult
    public func performUploadGame(data: GameEditRequest) async -> APIResult<GameEditResponseDTO>? {
        do {
            ...
            let decoder = JSONDecoder()
            decoder.dateDecodingStrategy = .iso8601
            return try decoder.decode(APIResult<GameEditResponseDTO>.self, from: responseData)
        } catch {
            print(error)
            return nil
        }
    ...
    private func refreshTokens() async -> Bool {
        guard let accessToken = tokenDataSource.accessToken,
